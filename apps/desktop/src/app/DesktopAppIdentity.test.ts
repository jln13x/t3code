          ...
          FileSystem.layerNoop({
            exists: (path) =>
              input.legacyPathProbeError
                ...
                : Effect.succeed(
                    input.legacyPathExists === true && path.includes("T3 Code (Fork)"),
                  ),
            readFileString: () =>
...
describe("DesktopAppIdentity", () => {
  ...
  it.effect("preserves failures while inspecting the legacy userData path", () => {
    const legacyPath = "/Users/alice/Library/Application Support/T3 Code (Fork)";
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["T3 Code (Fork)"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "T3 Code (Fork)");
