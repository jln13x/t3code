  ...
  Effect.gen(function* () {
    ...
    const startTurn = Effect.gen(function* () {
      ...
      return yield* Effect.gen(function* () {
        ...
        // On success the raw turn/started consumer clears this handoff. It may
        // run after turn/completed under load, so clearing it with the request
        // response would lose the authoritative id before projection sees it.
        Effect.onExit((exit) =>
          Exit.isFailure(exit) && pendingTurnStartId && input.pendingTurnStartIdRef
            ? Deferred.succeed(pendingTurnStartId, undefined).pipe(
                Effect.andThen(
                  Ref.update(input.pendingTurnStartIdRef, (current) =>
                    current === pendingTurnStartId ? undefined : current,
                  ),
                ),
              )
            : Effect.void,
    ...
    const handleRawNotification = (notification: CodexServerNotification) =>
      Effect.gen(function* () {
        ...
        if (notification.method === "turn/started") {
          ...
          if (pendingTurnStartId) {
            yield* Ref.update(pendingTurnStartIdRef, (current) =>
              current === pendingTurnStartId ? undefined : current,
            );
          }
        } else if (notification.method === "turn/completed") {
          // A start normally publishes turn/started first. If it did not,
          // completion is the last safe point to discard its response-id
          // handoff without letting it leak into a later turn.
          yield* Ref.set(pendingTurnStartIdRef, undefined);
        }
      ...
      sendTurn: (input) =>
        turnSubmissionSemaphore.withPermit(
          Effect.gen(function* () {
            ...
            let availableSkills: ReturnType<typeof parseCodexSkillsListResponse> | undefined;
            ...
            return yield* sendCodexTurn({
              ...
              // Derived from the session's own MCP configuration rather than
              // settings, so the prompt matches the tools this turn has.
